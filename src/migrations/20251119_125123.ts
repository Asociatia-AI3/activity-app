...
export async function up({ db, payload, req }: MigrateUpArgs): Promise<void> {
  ...
  await db.run(sql`CREATE INDEX \`posts_populated_authors_initiative_idx\` ON \`posts_populated_authors\` (\`initiative_id\`);`)
  ...
  await db.run(sql`CREATE INDEX \`_posts_v_version_populated_authors_initiative_idx\` ON \`_posts_v_version_populated_authors\` (\`initiative_id\`);`)
  ...
  await db.run(sql`CREATE TABLE \`guests\` (
  	...
  	\`name\` text,
  	\`organization\` text,
  	\`guest_type_speaker\` integer,
  	\`guest_type_workshop_holder\` integer,
  	\`guest_type_exhibitor\` integer,
  	\`bio\` text,
  	\`photo_id\` integer,
  	\`website\` text,
  	\`updated_at\` text DEFAULT (strftime('%Y-%m-%dT%H:%M:%fZ', 'now')) NOT NULL,
  ...
  `)
  await db.run(sql`CREATE INDEX \`schedules_edition_idx\` ON \`schedules\` (\`edition_id\`);`)
  await db.run(sql`CREATE INDEX \`schedules_activity_idx\` ON \`schedules\` (\`activity_id\`);`)
  await db.run(sql`CREATE INDEX \`schedules_location_idx\` ON \`schedules\` (\`location_id\`);`)
