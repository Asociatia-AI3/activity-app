...
export async function up({ db, payload, req }: MigrateUpArgs): Promise<void> {
  ...
  await db.run(sql`CREATE TABLE \`meetings\` (
  	...
  	\`title\` text NOT NULL,
  	\`date\` text NOT NULL,
  	\`value\` text DEFAULT 'workshop' NOT NULL,
  	\`workshop_topic\` text,
  ...
  await db.run(sql`CREATE TABLE \`ninjas\` (
  	...
  	\`guardian_email\` text,
  	\`guardian_phone\` text,
  	\`safety_agreements\` integer DEFAULT false NOT NULL,
  	\`photo_relase_agreement\` integer DEFAULT false NOT NULL,
  	\`updated_at\` text DEFAULT (strftime('%Y-%m-%dT%H:%M:%fZ', 'now')) NOT NULL,
  	\`created_at\` text DEFAULT (strftime('%Y-%m-%dT%H:%M:%fZ', 'now')) NOT NULL
  );
  `)
  ...
  await db.run(sql`CREATE TABLE \`locations_photos\` (
  	\`_order\` integer NOT NULL,
  	\`_parent_id\` integer NOT NULL,
  	\`id\` text PRIMARY KEY NOT NULL,
  	\`image_id\` integer,
  ...
  `)
  await db.run(sql`CREATE INDEX \`locations_photos_order_idx\` ON \`locations_photos\` (\`_order\`);`)
  await db.run(sql`CREATE INDEX \`locations_photos_parent_id_idx\` ON \`locations_photos\` (\`_parent_id\`);`)
  await db.run(sql`CREATE INDEX \`locations_photos_image_idx\` ON \`locations_photos\` (\`image_id\`);`)
...
export async function down({ db, payload, req }: MigrateDownArgs): Promise<void> {
  ...
  await db.run(sql`DROP TABLE \`locations_photos\`;`)
