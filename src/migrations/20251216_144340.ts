...
export async function up({ db, payload, req }: MigrateUpArgs): Promise<void> {
  ...
  await db.run(sql`CREATE TABLE \`activities\` (
  	...
  	\`audience_teens\` integer,
  	\`audience_adults\` integer,
  	\`audience_everyone\` integer,
  	\`section_id\` integer,
  	\`updated_at\` text DEFAULT (strftime('%Y-%m-%dT%H:%M:%fZ', 'now')) NOT NULL,
  	\`created_at\` text DEFAULT (strftime('%Y-%m-%dT%H:%M:%fZ', 'now')) NOT NULL,
  	FOREIGN KEY (\`edition_id\`) REFERENCES \`festival_editions\`(\`id\`) ON UPDATE no action ON DELETE set null,
  ...
  await db.run(sql`CREATE TABLE \`roles_permissions\` (
  ...
  `)
  await db.run(sql`CREATE INDEX \`roles_permissions_order_idx\` ON \`roles_permissions\` (\`order\`);`)
  await db.run(sql`CREATE INDEX \`roles_permissions_parent_idx\` ON \`roles_permissions\` (\`parent_id\`);`)
...
export async function down({ db, payload, req }: MigrateDownArgs): Promise<void> {
  ...
  await db.run(sql`DROP TABLE \`roles_permissions\`;`)
