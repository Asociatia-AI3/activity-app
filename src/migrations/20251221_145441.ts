...
export async function up({ db, payload, req }: MigrateUpArgs): Promise<void> {
  ...
  await db.run(sql`CREATE TABLE \`members\` (
  	...
  	\`type\` text,
  	\`sub_type\` text,
  	\`membership_fee_exempt\` integer DEFAULT false,
  	\`voting_rights\` integer DEFAULT false,
  	\`name\` text NOT NULL,
  	\`organization\` text,
  	\`photo_id\` integer,
  	\`updated_at\` text DEFAULT (strftime('%Y-%m-%dT%H:%M:%fZ', 'now')) NOT NULL,
  ...
  `)
  await db.run(sql`CREATE INDEX \`members_user_idx\` ON \`members\` (\`user_id\`);`)
  ...
  `)
  await db.run(sql`CREATE INDEX \`workshops_meeting_idx\` ON \`workshops\` (\`meeting_id\`);`)
  await db.run(sql`CREATE INDEX \`workshops_presenter_idx\` ON \`workshops\` (\`presenter_id\`);`)
  ...
  await db.run(sql`CREATE TABLE \`anti_workshops\` (
  	\`id\` integer PRIMARY KEY NOT NULL,
  	\`meeting_id\` integer NOT NULL,
  ...
  `)
  await db.run(sql`CREATE INDEX \`anti_workshops_meeting_idx\` ON \`anti_workshops\` (\`meeting_id\`);`)
  await db.run(sql`CREATE INDEX \`anti_workshops_updated_at_idx\` ON \`anti_workshops\` (\`updated_at\`);`)
  await db.run(sql`CREATE INDEX \`anti_workshops_created_at_idx\` ON \`anti_workshops\` (\`created_at\`);`)
  await db.run(sql`CREATE TABLE \`anti_workshops_rels\` (
  	\`id\` integer PRIMARY KEY NOT NULL,
  	\`order\` integer,
  	\`parent_id\` integer NOT NULL,
  	\`path\` text NOT NULL,
  	\`members_id\` integer,
  	FOREIGN KEY (\`parent_id\`) REFERENCES \`anti_workshops\`(\`id\`) ON UPDATE no action ON DELETE cascade,
  	FOREIGN KEY (\`members_id\`) REFERENCES \`members\`(\`id\`) ON UPDATE no action ON DELETE cascade
  );
  `)
  await db.run(sql`CREATE INDEX \`anti_workshops_rels_order_idx\` ON \`anti_workshops_rels\` (\`order\`);`)
  await db.run(sql`CREATE INDEX \`anti_workshops_rels_parent_idx\` ON \`anti_workshops_rels\` (\`parent_id\`);`)
  await db.run(sql`CREATE INDEX \`anti_workshops_rels_path_idx\` ON \`anti_workshops_rels\` (\`path\`);`)
  await db.run(sql`CREATE INDEX \`anti_workshops_rels_members_id_idx\` ON \`anti_workshops_rels\` (\`members_id\`);`)
  ...
  await db.run(sql`CREATE TABLE \`payload_locked_documents_rels\` (
  	...
  	FOREIGN KEY (\`anti_workshops_id\`) REFERENCES \`anti_workshops\`(\`id\`) ON UPDATE no action ON DELETE cascade,
  ...
  await db.run(sql`CREATE INDEX \`payload_locked_documents_rels_anti_workshops_id_idx\` ON \`payload_locked_documents_rels\` (\`anti_workshops_id\`);`)
  ...
  //  Migrarea optionala ce adauga un camp -'' phone '' in tabela ,, users ''
...
export async function down({ db, payload, req }: MigrateDownArgs): Promise<void> {
  ...
  await db.run(sql`DROP TABLE \`anti_workshops\`;`)
  await db.run(sql`DROP TABLE \`anti_workshops_rels\`;`)
