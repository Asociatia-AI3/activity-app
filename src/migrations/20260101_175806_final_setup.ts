...
export async function up({ db, payload, req }: MigrateUpArgs): Promise<void> {
  ...
  `)
  await db.run(sql`CREATE UNIQUE INDEX \`meetings_presenter_idx\` ON \`meetings\` (\`presenter_id\`);`)
  ...
  `)
  await db.run(sql`CREATE UNIQUE INDEX \`mentors_member_idx\` ON \`mentors\` (\`member_id\`);`)
  ...
  `)
  await db.run(sql`CREATE UNIQUE INDEX \`ninjas_member_idx\` ON \`ninjas\` (\`member_id\`);`)
