...
export async function up({ db, payload, req }: MigrateUpArgs): Promise<void> {
  ...
  await db.run(sql`CREATE TABLE \`locations\` (
  	...
  	FOREIGN KEY (\`photos_id\`) REFERENCES \`media\`(\`id\`) ON UPDATE no action ON DELETE set null,
  ...
  await db.run(sql`CREATE INDEX \`locations_photos_idx\` ON \`locations\` (\`photos_id\`);`)
