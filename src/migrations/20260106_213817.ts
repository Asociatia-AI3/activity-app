...
export async function up({ db, payload, req }: MigrateUpArgs): Promise<void> {
  ...
  `)
  await db.run(sql`CREATE UNIQUE INDEX \`members_user_idx\` ON \`members\` (\`user_id\`);`)
  ...
  await db.run(sql`CREATE TABLE \`festivaleditions\` (
  	\`id\` integer PRIMARY KEY NOT NULL,
  ...
  `)
  await db.run(sql`CREATE INDEX \`festivaleditions_updated_at_idx\` ON \`festivaleditions\` (\`updated_at\`);`)
  await db.run(sql`CREATE INDEX \`festivaleditions_created_at_idx\` ON \`festivaleditions\` (\`created_at\`);`)
  await db.run(sql`CREATE TABLE \`festivalsections\` (
  	\`id\` integer PRIMARY KEY NOT NULL,
  ...
  `)
  await db.run(sql`CREATE INDEX \`festivalsections_edition_idx\` ON \`festivalsections\` (\`edition_id\`);`)
  await db.run(sql`CREATE INDEX \`festivalsections_updated_at_idx\` ON \`festivalsections\` (\`updated_at\`);`)
  await db.run(sql`CREATE INDEX \`festivalsections_created_at_idx\` ON \`festivalsections\` (\`created_at\`);`)
...
export async function down({ db, payload, req }: MigrateDownArgs): Promise<void> {
  ...
  await db.run(sql`DROP TABLE \`festivaleditions\`;`)
  await db.run(sql`DROP TABLE \`festivalsections\`;`)
